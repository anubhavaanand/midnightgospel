...
const Trainworld: React.FC = () => {
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0, 0]} onClick={(e) => { e.stopPropagation(); openDialogue(8); }} onPointerOver={() => document.body.style.cursor = 'pointer'} onPointerOut={() => document.body.style.cursor = 'auto'}>
        <boxGeometry args={[2, 2, 2]} />
        <meshStandardMaterial color="#FFD700" wireframe />
      </mesh>

      <mesh>
        <cylinderGeometry args={[15, 15, 64, 64]} />
        {/* @ts-ignore */}
        <trainworldShaderMaterial ref={materialRef} side={THREE.BackSide} />
      </mesh>
