...
const CreamOcean: React.FC = () => {
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0, 0]} onClick={(e) => { e.stopPropagation(); openDialogue(3); }} onPointerOver={() => document.body.style.cursor = 'pointer'} onPointerOut={() => document.body.style.cursor = 'auto'}>
        <boxGeometry args={[2, 2, 2]} />
        <meshStandardMaterial color="#FFFDD0" wireframe />
      </mesh>

      <mesh>
        <planeGeometry args={[15, 64, 64]} />
        {/* @ts-ignore */}
        <creamOceanShaderMaterial ref={materialRef} side={THREE.BackSide} />
      </mesh>
