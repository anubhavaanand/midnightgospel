...
const SoulPrison: React.FC = () => {
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0, 0]} onClick={(e) => { e.stopPropagation(); openDialogue(5); }} onPointerOver={() => document.body.style.cursor = 'pointer'} onPointerOut={() => document.body.style.cursor = 'auto'}>
        <boxGeometry args={[2, 2, 2]} />
        <meshStandardMaterial color="#8A2BE2" wireframe />
      </mesh>

      <mesh>
        <cylinderGeometry args={[15, 64, 64]} />
        {/* @ts-ignore */}
        <soulPrisonShaderMaterial ref={materialRef} side={THREE.BackSide} />
      </mesh>
